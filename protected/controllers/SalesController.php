...
class SalesController extends Controller
{
	...
	public function actionCreate()
	{
		...
		{
			$model->attributes=$_POST['MSales'];
                        $model->create_time = date("Y-m-d H:i:s");
                        $model->create_login = Yii::app()->user->id;
                        if($model->save()){
//				$this->redirect(array('view','id'=>$model->id));
                                Yii::app()->user->setFlash('success',"Data Sales Tersimpan.");
	...
	{
            $this->menu_head = 'List Data Sales';
		$model=new MSales('search');
		$model->unsetAttributes();  // clear any default values
		if(isset($_POST['MSales'])){
                    $search = isset($_POST['MSales']['global_search'])?$_POST['MSales']['global_search']:null;
                    if($search){
                        $models = MSales::model()->globalSearch($search);
                        if(count($models)>0){
	 ...
	 * @return MSales the loaded model
	 * @throws CHttpException
	 */
	public function loadModel($id)
	{
		$model=MSales::model()->findByPk($id);
		if($model===null)
	 ...
	 * @param MSales $model the model to be validated
	 */
